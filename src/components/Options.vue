<template>
    <div class="h-full overflow-y-auto bg-zinc-100 dark:bg-zinc-800 rounded-md">
        <ul class="space-y-2 font-medium mt-5">
            <li class="px-2" @click="toggleDisplay(Feed)">
                <a href="#" class="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-zinc-200 dark:hover:bg-zinc-700 group">
           ...
           <li class="px-2" @click="toggleDisplay(PostManagement)">
              <a href="#" class="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-zinc-200 dark:hover:bg-zinc-700 group">
                 <svg class="w-5 h-5 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 21">
           ...
           <li class="px-2" @click="toggleDisplay(Bookmarks)">
              <a href="#" class="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-zinc-200 dark:hover:bg-zinc-700 group">
                <svg class="w-5 h-5 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 21">
         ...
         <li class="px-2" @click="requireConfirmation($event)">
            <a href="#" class="flex items-center p-2 text-red-600 hover:text-red-500 rounded-lg hover:bg-zinc-300 dark:hover:bg-zinc-700 group">
        ...
        <ConfirmPopup group="headless" class="border border-gray-300 dark-border">
            <template #container="{ message, acceptCallback, rejectCallback }">
                <div class="rounded p-4">
                    <span>{{ message.message }}</span>
                    <div class="flex items-center gap-2 mt-4">
                        <Button class="border border-red-600 p-1 text-red-600" label="Yes" @click="acceptCallback" size="small"></Button>
                        <Button class="border border-green-600 p-1 text-green-600" label="Cancel" outlined @click="rejectCallback" severity="secondary" size="small" text></Button>
                    </div>
                </div>
            </template>
        </ConfirmPopup>
...
import { Bookmarks, Feed, PostManagement } from '.';

const confirm = useConfirm();
const logOut = useLogout();
const { toggleDisplay } = usePostsStore();
