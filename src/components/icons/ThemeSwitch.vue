<template>
    <!-- <label class="inline-flex items-center relative cursor-pointer border border-zinc-400 rounded-full">
        <input class="peer hidden" id="toggle" type="checkbox" :checked="useMainStore().isDark" @click="useMainStore().toggleTheme" />
        <div
        ...
        <svg height="0" width="100" viewBox="0 0 24 24" data-name="Layer 1" id="Layer_1"
            xmlns="http://www.w3.org/2000/svg" class="fill-white peer-checked:opacity-60 absolute w-6 h-6 left-[13px]">
            <path
    ...
    <ToggleSwitch class="card flex justify-center" v-model="useMainStore().isDark" @click="useMainStore().toggleTheme" />
</template>
